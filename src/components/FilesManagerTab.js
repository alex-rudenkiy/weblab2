...
export function FilesManagerTab() {
    const onClick = ({ event, props }) => console.log(event,props);
// create your menu first
const MyAwesomeMenu = () => (
    <Menu id='menu_id'>
       <Item onClick={onClick}>Поделиться</Item>
       <Item onClick={onClick}>Удалить</Item>
       <Separator />
    ...
    let files=[]
    for (let i = 0; i < 300; i++) {
        files.push( <MenuProvider id="menu_id" style={{ border: '1px solid purple', display: 'inline-block' }}><ItemFigure filename="hello.txt"/></MenuProvider>);
    }
    return (
